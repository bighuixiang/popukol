	...
	<div class="max-content-box">
			...
			<div class="slider-box">
					<div class="user-login-box">
						<div class="login-box" v-show="!isLogin">
							...
							<input class="login-input" type="text"  v-model="form.email" placeholder="邮箱" v-on:keyup="isCanClickFn()">
        ...
        <el-tabs v-model="activeChannelCategory" @tab-click="handleClick">
          <el-tab-pane :key="index" v-for="(channelCategory,index) in channelCategorys" :label="channelCategory.name" :name="channelCategory.id+''">
              ...
              <div v-for="(item,index) in channelCategoryList" class="user-price-box">
                <a v-if="channelCategory.id!=1" :href="'//'+item.url" target="blank">
									<img class="avatar-box" :src="item.headImg" alt="">
									</a>
                <a v-else :href="'#/accoutdetails?uid='+item.id" target="blank">
                <img class="avatar-box" :src="item.headImg" alt="">
                </a>
                <i class="avatar-line"></i>
                <span class="name-text">{{item.name}}</span>
                <div class="text-center-box">
                    <span class="people-text"><i class="img-icon people-icon"></i>{{item.fans>=10000?(item.fans/10000).toFixed(2)+"w":item.fans}}</span>
      ...
      <div class="compentent-box compentent-hezuo">
        ...
        <el-carousel ref="carouselBottom" :interval="5000" height="230px" arrow="always" class="bottom-slide-box">
          <el-carousel-item v-for="(item,index_f) in homePartnerList" :key="index_f" style="padding-left: 6px;padding-right: 6px;">
            <el-row :gutter="20">
...
export default {
    ...
    methods: {
        ...
        getUserInfoFn() {
            let self = this;
            self.$http.get(self.API.userInfoApi, {}).then(
                response => {
                    // 响应成功回调
...
.login-btn-input input::-webkit-input-placeholder,
.login-btn-input::-webkit-input-placeholder {
    color: #666;
}
.send-code-btn-login {
    position: absolute;
